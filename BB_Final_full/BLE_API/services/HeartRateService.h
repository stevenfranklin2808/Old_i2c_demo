...
private:
    ...
    struct HeartRateValueBytes {
        static const unsigned MAX_VALUE_BYTES  = 1 + 2*NUM_VALUES_IN_PACKET; /* FLAGS + up to two bytes for heart-rate */
        ...
        void updateHeartRate(uint8_t hrmCounter) {
            ...
            valueBytes[FLAGS_BYTE_INDEX + 2] = (hrmCounter%5+1)*10;
        }
        ...
        void updateHeartRate(uint16_t hrmCounter) {
            ...
            valueBytes[FLAGS_BYTE_INDEX + 3] = (uint8_t) 123;
            valueBytes[FLAGS_BYTE_INDEX + 4] = (uint8_t) 0;
        }
        ...
        unsigned       getNumValueBytes(void) const {
            return 1 + ((valueBytes[FLAGS_BYTE_INDEX] & VALUE_FORMAT_FLAG) ? NUM_VALUES_IN_PACKET*sizeof(uint16_t) : NUM_VALUES_IN_PACKET*sizeof(uint8_t));
        }
